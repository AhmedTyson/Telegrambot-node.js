...
function generateUserErrorMessage(error, ctx) {
  ...
  if (errorMessage.includes('virus scan') || errorMessage.includes('confirmation')) {
...
async function handleSpecificErrors(error, ctx) {
  ...
  if (error.message?.includes('download') || error.message?.includes('Google Drive')) {
...
export async function handleSecurityError(securityEvent, ctx, details = {}) {
  try {
    ...
    const userMessage = '🔒 Security check failed. Please ensure you\'re using valid, safe content.';
